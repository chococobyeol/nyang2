...
test("keeps the mobile MML toolbar clear and the full editor reachable by touch", async () => {
  ...
  assert.match(mobileMml, /\.mml-studio \.mml-main-grid\.is-track-list-collapsed \{[^}]*grid-template-rows: 40px minmax\(0, 1fr\);/s);
  assert.match(mobileMml, /\.mml-studio \.mml-track-reorder-handle \{[^}]*position: static;[^}]*grid-column: 1;[^}]*transform: none;/s);
  assert.match(mobileMml, /\.mml-studio \.mml-track-list-title \{[^}]*width: 88px;[^}]*grid-template-columns: minmax\(0, 1fr\) 28px;[^}]*grid-template-rows: 26px 26px;[^}]*border-right:/s);
  assert.match(mobileMml, /\.mml-studio \.mml-track-list\.is-mobile-collapsed \.mml-track-list-title \{[^}]*width: 88px;[^}]*height: 32px;[^}]*grid-template-rows: 32px;/s);
  assert.match(mobileMml, /\.mml-studio \.mml-track-list-title \.mml-track-select-all \{[^}]*height: 26px;[^}]*grid-column: 1 \/ 3;[^}]*grid-row: 2;[^}]*align-self: center;[^}]*margin: 0;/s);
  assert.match(mobileMml, /\.mml-studio \.mml-track-card \{[^}]*width: 170px;[^}]*grid-template-columns: 8px 20px minmax\(0, 1fr\) 58px;[^}]*padding: 5px 8px;/s);
...
test("stacks the MML editor above the playable keyboard on portrait phones", async () => {
  ...
  assert.match(portraitMml, /\.app-viewport\.mml-open \.performance-surface \.top-bar \{[^}]*display: grid;[^}]*height: 110px;[^}]*grid-template-rows: 52px 52px;/s);
  assert.match(portraitMml, /\.app-viewport\.mml-open \.performance-surface \.top-bar \{[^}]*grid-template-columns: minmax\(0, 1fr\) 52px;[^}]*justify-content: stretch;/s);
  assert.match(portraitMml, /\.app-viewport\.mml-open \.performance-surface \.top-bar \.octave-area \{[^}]*grid-column: 1;[^}]*grid-row: 1;[^}]*justify-self: center;/s);
  assert.match(portraitMml, /\.app-viewport\.mml-open \.performance-surface \.top-bar \.transpose-panel \{[^}]*grid-column: 1 \/ 3;[^}]*grid-row: 2;/s);
